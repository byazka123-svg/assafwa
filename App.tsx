...
const App: React.FC = () => {
  ...
  const updateSelection = (cartKey: string, size: string, color: string) => {
    setCartItems(prev => prev.map(item => {
      if (item.cartKey === cartKey) {
        return { ...item, selectedSize: size, selectedColor: color };
      }
      return item;
    }));
  };
  ...
  const navigateTo = (view: 'home' | 'profile') => {
    setCurrentView(view);
      ...
      <>
        <div className="bg-brand-cream/60 border-b border-brand-cream py-2 md:py-3 overflow-hidden whitespace-nowrap">
          <div className="flex animate-marquee">
            <div className="flex items-center gap-12 md:gap-24 px-4 text-[9px] md:text-[11px] font-bold uppercase tracking-[0.3em] text-brand-brown">
              {[...Array(6)].map((_, i) => (
                <span key={i} className="flex items-center gap-3">
                  <svg className="h-4 w-4 text-brand-gold" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.17-2.81-.74-3.94-1.69-.15-.12-.29-.26-.43-.4-.02 2.75-.02 5.51-.02 8.26 0 1.45-.44 2.89-1.28 4.04-1.12 1.54-2.88 2.59-4.73 2.82-1.85.23-3.8-.18-5.32-1.26-1.52-1.07-2.58-2.77-2.86-4.6-.28-1.84.15-3.8 1.22-5.32 1.07-1.52 2.77-2.58 4.6-2.86.82-.12 1.65-.12 2.47-.02v4.04c-.66-.11-1.34-.06-1.97.16-.63.22-1.2.62-1.59 1.15-.39.53-.59 1.18-.56 1.83.03.65.29 1.28.73 1.75.44.47 1.05.77 1.71.84.66.07 1.32-.08 1.89-.42.57-.34 1.01-.86 1.24-1.48.23-.62.24-1.31.02-1.94V.02z"/>
                  </svg>
                  ASSAFWA SIGNATURE - PREMIUM QUALITY - EXCLUSIVE DESIGN - SHOP NOW ON TIKTOK LIVE EVERYDAY 16.00-21.00 WIB
                </span>
              ))}
            </div>
          </div>
        ...
        <section className="relative h-[25vh] md:h-[40vh] flex items-center justify-center overflow-hidden bg-brand-cream/20">
          ...
          <div className="z-10 text-center px-6">
            <span className="text-[8px] md:text-[10px] uppercase tracking-[0.6em] text-brand-gold font-bold mb-2 md:mb-4 block opacity-80">Collection 2024</span>
            <h2 className="text-2xl md:text-5xl font-bold brand-serif mb-2 md:mb-4 text-brand-black capitalize tracking-tight">
            ...
            <div className="flex justify-center items-center gap-4 md:gap-12 overflow-x-auto no-scrollbar">
              {categories.map(cat => (
                <button 
                  ...
                  className={`flex flex-col items-center gap-2 group transition-all duration-300 min-w-[60px] ${activeCategory === cat.id && currentView === 'home' ? 'text-brand-gold' : 'text-gray-400 hover:text-brand-black'}`}
                >
                  <div className={`p-2.5 rounded-full transition-all duration-500 ${activeCategory === cat.id && currentView === 'home' ? 'bg-brand-gold/10' : 'bg-gray-50'}`}>
                    <div className="scale-90 md:scale-100">{cat.icon}</div>
                  </div>
                  <span className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold whitespace-nowrap">
                    {cat.name}
                  </span>
                </button>
              ))}
          ...
          {loading ? (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-12">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="flex flex-col gap-4">
                  <div className="bg-brand-cream/40 aspect-[3/4] animate-pulse rounded-sm" />
                  <div className="h-4 bg-brand-cream/40 w-3/4 animate-pulse mx-auto" />
                  <div className="h-4 bg-brand-cream/40 w-1/4 animate-pulse mx-auto" />
                </div>
              ))}
            </div>
          ) : error ? (
            <div className="text-center py-20">
               <p className="text-red-400 uppercase tracking-widest text-xs font-bold">{error}</p>
               <button onClick={fetchProducts} className="mt-4 text-[10px] uppercase tracking-widest font-bold text-brand-gold underline">Coba Lagi</button>
            </div>
          ) : products.length > 0 ? (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-y-16 md:gap-x-12">
              {products.map((product) => (
                <div key={product.id} onClick={() => setSelectedProduct(product)} className="cursor-pointer">
                  <ProductCard 
                    product={product} 
                    onAddToCartClick={(p) => setProductToConfigure(p)} 
                  />
                </div>
              ))}
            </div>
            ...
            <div className="text-center py-20 flex flex-col items-center gap-4">
              <svg className="h-12 w-12 text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0l-8 4-8-4" />
              </svg>
              <p className="text-gray-400 uppercase tracking-widest text-[10px] font-bold">Belum ada koleksi yang tersedia untuk kategori "{activeCategory}".</p>
            </div>
          )}
        </main>
      </>
  ...
  return (
    <div className="min-h-screen bg-brand-white selection:bg-brand-gold selection:text-white pb-20 md:pb-0">
      <div className="bg-brand-black py-2.5 text-center">
        <div className="max-w-7xl mx-auto px-4 text-[8px] md:text-[9px] uppercase tracking-[0.2em] text-brand-gold font-bold">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-brand-cream/50 h-[70px] flex items-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 w-full flex items-center justify-between">
          <div className="flex flex-col cursor-pointer" onClick={() => navigateTo('home')}>
            <h1 className="text-xl md:text-3xl font-bold tracking-[0.2em] brand-serif text-brand-black uppercase">ASSAFWA</h1>
          </div>

          <nav className="hidden md:flex items-center gap-10 text-[10px] uppercase tracking-[0.3em] font-bold">
            ...
            <button className="text-brand-black hover:text-brand-gold transition-colors">Lokasi Store</button>
          </nav>
      ...
      <div className={`fixed top-0 right-0 h-full w-[280px] bg-white z-[101] shadow-2xl transform transition-transform duration-500 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-6 border-b border-brand-cream flex justify-between items-center">
          <h2 className="brand-serif font-bold text-lg tracking-widest uppercase">Menu</h2>
        ...
        <nav className="p-8 flex flex-col gap-8 text-[11px] uppercase tracking-[0.3em] font-bold">
          ...
          <button className="text-left py-2 border-b border-brand-cream/30 hover:text-brand-gold">Bantuan</button>
          <button className="text-left py-2 border-b border-brand-cream/30 hover:text-brand-gold">Lokasi Store</button>
        </nav>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
            <div className="space-y-8 text-center md:text-left">
              <h2 className="text-3xl font-bold brand-serif text-brand-gold uppercase tracking-[0.3em]">ASSAFWA</h2>
              <p className="text-gray-400 text-[11px] leading-relaxed tracking-wider uppercase opacity-60 max-w-xs mx-auto md:mx-0">
                Menyediakan busana muslim premium dengan kualitas kain terbaik dan desain eksklusif sejak 2018.
              </p>
            </div>
            <div className="text-center md:text-left">
              <h3 className="text-brand-gold text-[10px] uppercase tracking-[0.3em] font-bold mb-8">Katalog</h3>
              <ul className="space-y-4 text-[11px] uppercase tracking-[0.15em] text-gray-400 font-medium">
                <li><button onClick={() => handleCategorySelect('koko')} className="hover:text-white">Koleksi Koko</button></li>
                <li><button onClick={() => handleCategorySelect('kurta')} className="hover:text-white">Koleksi Kurta</button></li>
                <li><button onClick={() => handleCategorySelect('jubah')} className="hover:text-white">Koleksi Jubah</button></li>
              </ul>
            </div>
            <div className="text-center md:text-left">
              <h3 className="text-brand-gold text-[10px] uppercase tracking-[0.3em] font-bold mb-8">Bantuan</h3>
              <ul className="space-y-4 text-[11px] uppercase tracking-[0.15em] text-gray-400 font-medium">
            ...
            <div className="text-center md:text-left">
              <h3 className="text-brand-gold text-[10px] uppercase tracking-[0.3em] font-bold mb-8">Offline Store</h3>
              <p className="text-[10px] text-white/60 leading-relaxed uppercase">Pertokoan Assafwa, Blok C-04, Tanah Abang, Jakarta Pusat</p>
            </div>
          </div>
          <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
            <p className="text-gray-600 text-[9px] uppercase tracking-[0.4em] font-bold text-center md:text-left">
              &copy; 2024 ASSAFWA SIGNATURE. ALL RIGHTS RESERVED.
            </p>
          </div>
        </div>
      </footer>
