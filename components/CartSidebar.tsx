...
export const CartSidebar: React.FC<CartSidebarProps> = ({ isOpen, onClose, items, onRemove, onUpdateQty, onUpdateSelection }) => {
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 bg-brand-black/40 backdrop-blur-sm z-40 transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />
              ...
              items.map(item => (
                <div key={item.cartKey} className="flex gap-4 items-start animate-fadeIn border-b border-brand-cream pb-6">
                  ...
                  <div className="flex-grow">
                    <h4 className="font-medium text-brand-black text-[14px] tracking-tight mb-2">{item.name}</h4>
        ...
        <div className="p-6 border-t border-brand-cream bg-white">
          ...
          {showForm ? (
            <button 
              ...
              className="w-full py-4 bg-green-600 text-white font-bold uppercase tracking-widest text-[10px] transition-all hover:bg-green-700 shadow-xl flex items-center justify-center gap-2"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12.031 6.172c-2.32 0-4.591.905-6.229 2.543-1.638 1.639-2.543 3.909-2.543 6.229 0 2.37.935 4.689 2.57 6.323l.115.115-.991 3.621 3.704-.972.115.067c1.554.918 3.324 1.399 5.129 1.401h.001c2.321 0 4.591-.905 6.23-2.543 1.639-1.638 2.544-3.908 2.544-6.228 0-2.321-.905-4.591-2.543-6.23-1.638-1.638-3.908-2.543-6.228-2.543zm0 2.031c1.868 0 3.633.727 4.965 2.059 1.332 1.332 2.059 3.097 2.059 4.965 0 1.868-.727 3.633-2.059 4.965-1.332 1.332-3.097 2.059-4.965 2.059h-.001c-1.516 0-3-.418-4.281-1.21l-.307-.182-2.147.564.574-2.094-.2-.319c-.866-1.378-1.321-2.981-1.321-4.633 0-1.868.727-3.633 2.059-4.965 1.332-1.332 3.097-2.059 4.965-2.059z"/>
              </svg>
              Kirim Pesan
            </button>
          ) : (
