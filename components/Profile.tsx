...
export const Profile: React.FC<ProfileProps> = ({ onBack, onLogout }) => {
  ...
  const userInitial = user.username.substring(0, 2).toUpperCase();
      ...
      <div className="relative overflow-hidden bg-brand-black rounded-sm p-6 md:p-10 text-white mb-10 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-brand-gold rounded-full flex items-center justify-center text-white text-2xl font-medium border-4 border-brand-black shadow-xl">
              {userInitial}
            </div>
            <div>
        ...
        {activeTab === 'orders' && (
          <div className="space-y-4 animate-fadeIn">
            {mockOrders.map((order) => (
             ...
             <p className="text-center text-sm text-gray-400 pt-8">Belum ada riwayat pesanan.</p>
          </div>
        )}
      </div>
